#!/usr/bin/env python3
"""
train_camus_3b.py — fine-tune Llama-3.2-3B-Instruct into the Camus persona.

Same recipe as the 8B (two SFT passes: voice, then "Step 3.5" refusals), smaller base so
it runs fast on free CPU after deployment. Uses Unsloth (fits a FREE Colab T4, exports GGUF).

EDIT ONLY THE CONFIG BLOCK: point STAGE1_FILES / STAGE2_FILES at the SAME .jsonl files you
used for the 8B. The loader accepts prompt/response, prompt/completion, instruction/output,
messages, or sharegpt conversations — no reformatting needed.

RUN (Colab):  !pip install unsloth   then   !python train_camus_3b.py
OUTPUT:       ./camus-3b-gguf/camus-3b.Q4_K_M.gguf   (upload to the Hub)
"""
...
LEARNING_RATE = 2e-4
QUANT        = "q4_k_m"
OUT_MERGED   = "camus-3b-merged"
OUT_GGUF     = "camus-3b-gguf"
# ===========================================================================
